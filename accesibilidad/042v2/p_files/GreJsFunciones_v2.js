	...
	/*function sanitizar()
	{
		const modelo = $("input[id='modelo']").val();
		
		if(modelo!=null && modelo!='401' && modelo!='403'&&  modelo!='801_2021' &&
		   modelo!='600_2009' && modelo!='610' && modelo!='620_2025' && modelo!='620_2026' && modelo!='630' && modelo!='651_2009' && modelo!='655_2009')
		{
			sanitizarInputs();
			sanitizarTagsQueNoSonInputs();
		}		
	}	
	
	
	// PROBLEMAS: 
	// - Al actualizar el modelo 400 simplemente por cambiar la opcion PRESENCIAL o TELEMATICA del combo ya no se cargaba la pagina. Tras eliminar varios tags 
	// para sanitizar, eso ya funciona pero al completar el modelo 400 entero e imprimir el desplegable de UNIDAD no se muestra al VALIDAR
	// - El modelo 600 falla (error de pagina no encontrada)al usar el DatePicker pero curiosamente ocurre cuando lo actualizamos despues de seleccionar primero la oficina liquidadora.
	//   Si lo hacemos al reves parece no fallar  
	function sanitizarTagsQueNoSonInputs() 
	{
	    // Selecciona todos los elementos que podrían contener contenido dinámico
	    //const elementsToSanitize = document.querySelectorAll('input, textarea, div, span, a, img, iframe, form, p, label, table, tr, td, th');

		//NOTA: se eliminan de la lista original propuesta por la IA los tags div y form porque no funcionan correctamente y los tags input y textarea porque ya se cubren en sanitizarInputs()
		const elementsToSanitize = document.querySelectorAll('span, a, img, iframe, p, label, table, tr, td, th'); 
	
	    elementsToSanitize.forEach(element => {
			if (element.tagName === 'IMG') {
	            element.src = DOMPurify.sanitize(element.src);
	        } else if (element.tagName === 'A') {
	            element.href = DOMPurify.sanitize(element.href);
	        } else {
	            element.innerHTML = DOMPurify.sanitize(element.innerHTML);
	        }
	    });
	
	    console.log("Todos los elementos han sido sanitizados.");
	}
	
	
	function sanitizarInputs()
	{
		const vInputElements = document.querySelectorAll('input');
		for (const vInput of vInputElements) 
		{					  	
			const id = vInput.id;
			const valor = vInput.value;
			
			//alert("id='" + id + "'. valor='" + valor + "'");
						
			//if(id!='posicionScroll' && id!='posicionFoco' && id!='' && id!='modelo' && id!='validar' && id!='idListaSesion' && id!='idBeanSesion'
			//   && id!='almacenarsp' && id!='0')
			//{
				const valorSanitizado = DOMPurify.sanitize(valor);				
				$('input[id="' + id + '"]').val(valorSanitizado);
			//}												  
		}  		
	}	
	
		
	// No funciona correctamente. Todavia no borro esta función como referencia de lo probado o por si se nos ocurre alguna modificacion que funcione	
	function sanitizarBody()
	{
		// Seleccionar el contenido del body
		const bodyContent = document.body.innerHTML;
		
		// Sanitizar el contenido
		const sanitizedContent = DOMPurify.sanitize(bodyContent);
		
		// Reemplazar el contenido del body con el contenido sanitizado
		document.body.innerHTML = sanitizedContent;
		
		console.log("Contenido del body sanitizado:", sanitizedContent);		
	}*/
